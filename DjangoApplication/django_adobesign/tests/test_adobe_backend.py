...
def test_get_adobesign_participants_in_right_order(minimal_signature,
                                                   adobe_sign_backend):
    signer1 = Signer(full_name='Poney poney', email='thomas.johnsan@okstate.edu',
                     signing_order=1)
    signer2 = Signer(full_name='Pouet pouet', email='sesuarez@okstate.edu',
                     signing_order=2)
    ...
    expected_signer1 = {
        'name': 'Thomas',
        'memberInfos': [
            {'email': 'thomas.johnsan@okstate.edu'}],
        'order': 1,
        'role': 'SIGNER'
    }
    expected_signer2 = {
    ...
    mocker.patch.object(
        ...
        return_value={
            'participantSets': [
                ...
                {'memberInfos': [{'email': 'thomas.johnsan@okstate.edu'}],
                 'id': 'fooid',
                 'status': 'NOT_YET_VISIBLE',
                 'order': 2},
                {'memberInfos': [{'email': 'sesuarez@okstate.edu'}],
                 'id': 'barid',
                 'status': 'CANCELLED',
                 'order': 1}]})

    # Signers
    signer1 = Signer(full_name='Thomas', email='thomas.johnsan@okstate.edu',
                     signing_order=1)
    signer2 = Signer(full_name='Salome', email='sesuarez@okstate.edu',
                     signing_order=2)
    signer2.signature = minimal_signature
    signer1.signature = minimal_signature
    signer2.save()
...
def test_get_next_signer_url(mocker, adobe_sign_backend):
    ...
    assert (mail, url) == ('fran@okstate.edu', 'url')
...
def test_map_adobe_signer_to_signer(mocker, adobe_sign_backend,
    ...
    signer1 = Signer(full_name='Thomas', email='thomas.johnsan@okstate.edu',
                     signing_order=1)
    signer2 = Signer(full_name='Thomas', email='thomas.johnsan@okstate.edu',
                     signing_order=2)
    signer3 = Signer(full_name='Thomas', email='thomas.johnsan@okstate.edu',
                     signing_order=3)
